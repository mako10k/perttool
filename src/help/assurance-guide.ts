  ...
  Object.freeze({
    ...
    actuals: Object.freeze({
      sections: Object.freeze({
        ...
        "read-only-observation": "project history remains bound to selected first-parent Git evidence. --history-provenance automatic keeps rename inference fail-closed; new-root explicitly treats the selected path as a new root only when the excluded predecessor is readable and has a different project identity. Both history and velocity results expose the same evidence. project observe-velocity uses the exact current operand for declared evidence, the selected revision for git-recorded evidence, and returns both as separate candidates for all. The top source digest and nested history source digest identify those inputs independently. No observation changes the source, Git, or project.velocity.",
