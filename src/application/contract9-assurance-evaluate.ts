...
function bound(
  ...
  const value = bounds.find((item) =>
    item.entityId === taskId && item.event === event && item.direction === direction
  );
...
function contractV2(
  ...
  return Object.freeze({
    ...retained,
    model: "Perttool.TaskPlanContract.v2",
    when: Object.freeze({
...
export function evaluateContract9PlanAssurance(
  text: string,
): PlanAssuranceEvaluationV1 | null {
  ...
  const tasks = input.tasks.map((task) => Object.freeze({
    ...task,
