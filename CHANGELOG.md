...
milestone outcome acceptance as a separate fact from graph closure.
...
the bundled result schemas and adds selectable full and reference-based
outline projections without changing existing result identities or runtime
payload meanings.

### Added

- Added full and reference-based outline views to `perttool schema`, including
  separate lookup of one bundled `$ref` detail without changing default
  complete-artifact output.

### Fixed

- Replaced underspecified nested JSON Schema object placeholders with complete
  closed record definitions and added nested unknown-field rejection coverage.
