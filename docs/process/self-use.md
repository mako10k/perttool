...
- Document status: Active Stage 3 / Revision 2.47
...
- scheduling-and-units milestone planning gate: `TIME-001` and `UNIT-001` map to SU-M0 through SU-M5 without duplicating detail task state; after the current SU-M2 detail rollup at six decimal day precision, the provisional macro precedence/resource makespans are `4.411765d` and `6.411765d` with 2d resource delay, and `SU_M2_TEMPORAL_SURFACE_WORK_PACKAGE` is recommended
