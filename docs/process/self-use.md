...
Complete `NextResult.v3` recommends `M1_CONTRACT_REVIEW`. The six-decimal macro
rollup produces `6.090909d` precedence and `8.090909d` heuristic resource
makespans with 2d resource delay.
...
- scheduling-and-units milestone planning gate: `TIME-001` and `UNIT-001` map to SU-M0 through SU-M5 without duplicating detail task state; after the normative-examples rollup, the provisional macro precedence/resource makespans are `6.090909d` and `8.090909d` with 2d resource delay, and `SU_M1_CONTRACT_WORK_PACKAGE` is recommended
