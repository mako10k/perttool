...
disposable directory, and exercises the extracted server and DAG assets. It
neither installs nor publishes the extension.
...
The first suffix-free beta, `v0.1.0`, is [accepted](beta-release-acceptance.md). One tarball was verified across the GitHub prerelease, npm `beta`, and an isolated registry installation. It was then explicitly promoted to npm `latest`; both tags resolve to `0.1.0`. The macro plan is advanced to `M8_BETA_RELEASED` and has no ready or recommended task. Issue #3 multi-plan composition remains a post-beta backlog. The later selected `ADAPTER-001` plan composes the shared dependency, library, Node Host, CLI parity, document-session, read-only LSP, VSIX/DAG, and read-only MCP gates; its architecture contract, Core dependency cleanup, additive Core/Node shared-library boundary, editor protocol contract, protocol-neutral document-session Core, private read-only LSP Core, isolated LSP acceptance, private VSIX shell, read-only DAG Webview, six-port Node Host boundary, CLI facade parity, read-only MCP contract, private MCP implementation, and isolated MCP acceptance are complete. The LSP workspace pins stable protocol 3.17.5, uses local stdio and exact document-session projections, stays outside the public package, and passes the isolated dual-tarball Node.js 22 gate. The current fourteen-file VSIX fixes VS Code `^1.101.0`, exact language client 9.0.1, TextMate presentation, untrusted/virtual workspace support, version-bound virtual Help, an offline bundled server, and a restrictive GraphView-only DAG Webview with four modes, closed messages, source navigation, and an accessible exact-value outline. The current exact portable Core has 45 runtime values and a 34-module closure while root/Node remain exact 122-name facades with one reference-identical `createNodeHost()` factory. The CLI composes one private Application facade over that Host, routes document/digest, Git evidence, artifact, and Grammar 6 persistence through injected ports, and retains all 44 Contract 7 commands. The MCP workspace implements final revision `2026-07-28`, exact stable server SDK `2.0.0`, modern-only local stdio, four immutable resources, five read-only tools, digest-bound local sources, self-contained schemas, and hard limits, strict malformed-line fail-closure, and isolated two-client execution through direct Application calls without Git, persistence, or a CLI subprocess. Two tasks and 9p remain with equal 9p precedence/resource makespans and zero resource delay. Complete NextResult v6 recommends and makes startable only `VSIX_ACCEPTANCE`; `ADAPTER_INTEGRATION_ACCEPTANCE` remains upcoming.
