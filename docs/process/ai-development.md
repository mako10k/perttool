...
- Editor protocol contract: [../specs/editor-protocol.md](../specs/editor-protocol.md)
...
`npm run check:mcp-package` creates separate root and private-MCP tarballs,
installs both with scripts disabled, and exercises modern stdio, discovery,
registered source/digest behavior, failures, two-client parity, and unchanged
project bytes from a disposable non-Git directory. It publishes nothing and
does not expose MCP from the public root package.

`npm run check:vsix-shell` bundles the exact language client and accepted
server into the private extension, creates the fourteen-file VSIX in a
disposable directory, exercises the extracted server and DAG assets, and uses
...
The first suffix-free beta, `v0.1.0`, is [accepted](beta-release-acceptance.md). One tarball was verified across the GitHub prerelease, npm `beta`, and an isolated registry installation. It was then explicitly promoted to npm `latest`; both tags resolve to `0.1.0`. The macro plan is advanced to `M8_BETA_RELEASED` and has no ready or recommended task. Issue #3 multi-plan composition remains a post-beta backlog. The later selected `ADAPTER-001` plan composes the shared dependency, library, Node Host, CLI parity, document-session, read-only LSP, VSIX/DAG, and read-only MCP gates; its architecture contract, Core dependency cleanup, additive Core/Node shared-library boundary, editor protocol contract, protocol-neutral document-session Core, private read-only LSP Core, isolated LSP acceptance, private VSIX shell, read-only DAG Webview, supported-host VSIX acceptance, six-port Node Host boundary, CLI facade parity, read-only MCP contract, private MCP implementation, isolated MCP acceptance, and final cross-surface integration acceptance are complete. The LSP workspace pins stable protocol 3.17.5, uses local stdio and exact document-session projections, stays outside the public package, and passes the isolated dual-tarball Node.js 22 gate. The current fourteen-file VSIX fixes VS Code `^1.101.0`, exact language client 9.0.1, TextMate presentation, untrusted/virtual workspace support, version-bound virtual Help, an offline bundled server, and a restrictive GraphView-only DAG Webview with four modes, closed messages, source navigation, and an accessible exact-value outline. Its installed gate uses exact test-electron 3.1.0 to prove trusted/untrusted minimum-host activation, virtual and large graphs, Help/navigation, replacement, uninstall readback, and source identity. The current exact portable Core has 45 runtime values and a 34-module closure while root/Node remain exact 122-name facades with one reference-identical `createNodeHost()` factory. The CLI composes one private Application facade over that Host, routes document/digest, Git evidence, artifact, and Grammar 6 persistence through injected ports, and retains all 44 Contract 7 commands. The MCP workspace implements final revision `2026-07-28`, exact stable server SDK `2.0.0`, modern-only local stdio, four immutable resources, five read-only tools, digest-bound local sources, self-contained schemas, and hard limits, strict malformed-line fail-closure, and isolated two-client execution through direct Application calls without Git, persistence, or a CLI subprocess. Sixteen dependency-ordered integration cases prove exact cross-surface semantics, protocol binding, diagnostic ownership, package isolation, supported-host behavior, complete self-use coverage, and no-write side effects. All sixteen tasks and 91p are complete and retained before advance, both remaining makespans are zero, resource delay is zero, and complete NextResult v6 has no recommendation or startable task.

The separately selected `EDITOR-MUTATION-001` plan adds an opt-in Editor
Protocol model 2 through dependency-ordered gates. Its accepted
[Tiered Editor Mutation Contract](../specs/editor-mutations.md) classifies one
complete final candidate by the strictest of `E0` semantic equivalence, `E1`
completely unsealed repair, `E2` non-destructive recoverable edit, and `E3`
authority-sensitive assurance, governance, destructive, or advance work.
The contract task is complete without activating a capability: the current
private LSP still selects model 1 and exposes no document formatter. The next
normal task is `EDITOR_FORMAT_CORE`; later mutation classes, release, public
VSIX publication, remote writes, Issue changes, and plan advance remain
separately gated.
