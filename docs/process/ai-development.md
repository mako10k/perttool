...
2026-07-22の[Recommendation設計受け入れ](recommendation-design-review.md)、grammar受け入れ、`M1_ROADMAP_UPDATE`、`TASK_MUTATION_CORE`、`ENTITY_MUTATION_CORE`は完了した。現在のmacro CPは`FORMATTER_CORE`である。対応する[操作系詳細plan](../../plans/operations.pert)ではprecedence criticalな`FORMAT_APPLICATION`と、`CLI_SURFACE`/`REVIEWERS`のresource arcを駆動するschedule criticalな`MUTATION_CLI_PREVIEW`が同時に`runnable_now`である。Forecastを維持するにはdeveloper capacity 2で両方を並行開始する。Recommendation実装とIssue #2は`src/cli.ts`、`src/index.ts`、reviewerを操作系と共有するため、`M3_SAFE_WRITE_READY`より前には並行実装しない。
