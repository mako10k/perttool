...
- [temporal-units.md](temporal-units.md): target Grammar 2/3 and Contract 4 boundary cases for calendar projection, deadlines, start authority, exact Decimal-or-fraction Point/time migration, and deterministic text/JSON

`.pert` files in this directory are normative samples for grammar version 1, semantics version 1, and analysis version 1. Target Grammar 2 source fixtures and the Grammar 3 machine acceptance cases live under `test/fixtures/temporal-units/` and remain non-runnable by the current Contract 3 package until the atomic Contract 4 cutover. `recommendation.md` contains normative cases for the implemented Recommendation interface version 1, and `agent-guidance.md` contains normative cases for Agent Guidance interface version 1 ahead of later Core/CLI implementation. `mermaid-profile.md` is the Mermaid adapter wire contract and export golden; tests fix the byte output of `exportMermaid` and `dag render --to mermaid`.
