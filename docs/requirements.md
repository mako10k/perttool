...
[ADR 0003](adr/0003-beta-versioning.md)により、最初のbetaをsuffixなし`0.1.0`、以後の`0.x.x`をbetaと定義した。Beta gateへ追加したIssue #2のread-only AI Agent Guidance Registry v1は[受け入れ済み](process/agent-guidance-acceptance.md)である。5 provider baselineを公式資料からoffline design inputへ固定し、AI Agent Guidance Registry仕様と規範例で公開contractを確定した。Version付きoffline profile、validator、query、index/quick/detail projection、deterministic JSON/text、structured command help、`agent help` CLI、package-installed Core/CLI parity、security境界を全checkで検証した。現在の工程はmacroの`BETA_RELEASE_E2E`だけで、Issue #3、LSP server、VSIX、MCP serverはbeta blockerに含めない。
