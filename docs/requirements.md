...
Analysis実装は`dag next`まで進んでいる。Exact Rational、PERT expected/variance、precedence CPM、critical path count、決定的resource schedule、capacity override、resource arc、schedule critical pathに加え、next分類、`runnable_now`、resource rejection、upcoming explanationをtext/JSONで返せる。Slice 2のbootstrap gate、grammar acceptance、safe-write gate、advance gateを満たし、macro `plans/mvp.pert`と詳細`plans/grammar.pert`、`plans/control-plane.pert`、`plans/operations.pert`でStage 3のpreview-first advance自己利用を行っている。Issue #1のproduct vision、要件境界、実行可否と推奨度model、ranking policy、reason code taxonomy、structured explanation、Core/text/JSON interface、human override contract、normative example、test観点、self-useと実装migration方針は[横断設計レビュー](process/recommendation-design-review.md)で受け入れた。これは設計完了であり、recommendation機能の実装完了ではない。`M1_ROADMAP_UPDATE`で操作系を詳細化した後、task/milestone/resource add/set/remove、task finish、atomic batchをpure library Coreへ実装し、formatter/mutation previewとsafe writeをtext/JSONへ公開した。Canonical keep/remove set、partial join条件、前後のeffective reached・ready一致、idempotenceを再検査する`planAdvance` Coreと、preview、diff、advance固有JSON、safe `--write`/`--out`/`--expect-digest`を持つ`dag advance` CLIも実装済みである。操作系24pは完了し、実測Velocityは`24p/1d`、残作業とforecastは0である。Mermaid profileとexport、macro `ADVANCE`を完了し、macroの残るprecedence/resource makespanはともに7d、resource delayは0dである。唯一のreadyかつ`runnable_now`なcritical work packageは`MERMAID_ROUNDTRIP`である。Recommendation実装とIssue #2のAI Agent Guidance RegistryはM3後に詳細化可能だが、macro planへ追加するまでは着手順を推測しない。Issue #3のbacklog階層・multi-plan compositionは独立した将来backlogとして保持する。
