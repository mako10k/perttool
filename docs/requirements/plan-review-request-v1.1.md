...
- Status: Candidate 1.1
- Date: 2026-09-10
- Authority source: GitHub Issue #21, the owner's instruction to address it,
  the completed Candidate 1.0 independent review, and the owner's
  `REVISE` decision
- Prior candidate: Candidate 1.0 remains unchanged as the reviewed historical
  snapshot
...
## Revision from Candidate 1.0

Candidate 1.1 changes only the four findings from the completed independent
review:

- resolving one request clears `review_required` only when no other request
  remains open;
- CLI Contract 11 uses `Perttool.NextResult.v9` for supported Grammar 1
  through 10 inputs while preserving legacy meanings rather than the v8
  envelope identity;
- `plan_changed` means that a separately authorized and persisted plan
  change already exists before resolution; and
- resolution uses a distinct Plan Review authority projection rather than
  classifying the operation as an existing DAG mutation.

No other Candidate 1.0 behavior, acceptance criterion, or scope changes.
...
without a plan mutation. `plan_changed` means that a separately authorized,
governed, validated, and persisted plan mutation already exists in the exact
pre-resolution source. It is not a future intention to change the plan and it
is not proof that the plan change was correct. A request remains open until
the selected disposition is true.
...
the exact original, candidate, Plan Review authority decision, request
identity, and write outcome. Under CLI Contract 11,
...
- `plan_changed` requires the separately governed and persisted plan change
  to exist in the bound pre-resolution source; an intended future change
