...
- Active CLI contract version: 10
- Created: 2026-07-30
...
every result identity advertised by the active Contract 10 command registry
...
`cli_contract_version=10` for CLI envelopes, and rejects unknown root fields.
...
| `Perttool.AnalysisResult.v7` | `dag analyze` result with calendar scheduling and plan assurance |
| `Perttool.AdvanceResult.v4` | history-, assurance-, acceptance-, and planning-aware `dag advance` result |
| `Perttool.CheckResult.v6` | `document check` result with temporal and planning validation |
...
| `Perttool.MilestoneAcceptanceMigrationResult.v1` | committed Grammar 7 migration result |
| `Perttool.MilestoneAcceptanceResult.v1` | milestone criterion and receipt inspection result |
| `Perttool.MutationResult.v6` | direct, lifecycle, batch, assurance, acceptance, and temporal mutation result |
| `Perttool.NextResult.v8` | next-task result with temporal and assurance-aware start authority |
| `Perttool.PlanAssuranceResult.v2` | model-2 assurance show and pinpoint hash result |
| `Perttool.PlanningMutationResult.v1` | planning reshape and Window mutation result |
| `Perttool.PlanningPoolResult.v1` | Work and Window read, current observation, and historical observation result |
| `Perttool.PlanningReshapePreflightResult.v1` | normalized, audited, hash- and token-bound reshape preflight result |
| `Perttool.ProjectHistoryResult.v1` | read-only project history result |
| `Perttool.ProjectResult.v5` | project metadata with calendar-aware temporal source identity |
| `Perttool.SchemaResult.v1` | schema catalog and lookup result |
| `Perttool.UnitMigrationResult.v5` | exact complete DAG and Activity unit-migration result |
...
The twenty-five command identities are command results. OverrideDecision is not a
