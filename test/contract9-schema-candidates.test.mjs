...
test("staged Contract 9 schemas contain no unspecified object shape", async () => {
  const failures = [];
  for (const identity of identities) {
    const value = await schema(path.join(candidateDirectory, `${identity}.schema.json`));
...
test("real Grammar 8 temporal projections satisfy the staged nested contracts", async () => {
  const ajv = new Ajv2020({ allErrors: true, strict: true });
