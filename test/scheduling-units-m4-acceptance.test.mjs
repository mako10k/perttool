...
test("the active package root hides target helpers while Contract 6 retains migration", async () => {
  ...
  assert.equal(helpJson.cli_contract_version, 10);
  assert.equal(guideJson.cli_contract_version, 10);
  assert.equal(help.stdout.includes("project migrate-unit"), true);
  assert.equal(help.stdout.includes("Perttool.UnitMigrationResult.v5"), true);
  assert.equal(
  ...
  assert.equal(result.schema_version, "Perttool.UnitMigrationResult.v5");
