...
test("advance history safety plan matches the ADV-001 delivery roadmap", async () => {
  const text = await readFile(
    path.join(root, "plans/advance-history-safety.pert"),
    "utf8",
  );
  const expected = JSON.parse(await readFile(
    path.join(
      testDirectory,
      "golden/self-use/advance-history-safety.expected.json",
    ),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
...
test("repository-clean advance plan matches the ADV-002 delivery roadmap", async () => {
  const text = await readFile(
    path.join(root, "plans/advance-clean-candidate.pert"),
    "utf8",
  );
  const expected = JSON.parse(await readFile(
    path.join(
      testDirectory,
      "golden/self-use/advance-clean-candidate.expected.json",
    ),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
...
test("0.5.0 release plan matches the Contract 6 release roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.5.0.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.5.0.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});
...
test("0.5.2 release plan matches the complete JSON Schema patch roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.5.2.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.5.2.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});

test("0.5.3 release plan matches the governance guidance patch roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.5.3.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.5.3.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});

test("0.5.4 release plan matches the governance runtime warning patch roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.5.4.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.5.4.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});

test("0.5.5 release plan matches the governed-preview warning patch roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.5.5.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.5.5.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});
...
test("0.7.0 release plan matches the conditional plan assurance roadmap", async () => {
  const text = await readFile(path.join(root, "plans/release-0.7.0.pert"), "utf8");
  const expected = JSON.parse(await readFile(
    path.join(testDirectory, "golden/self-use/release-0.7.0.expected.json"),
    "utf8",
  ));
  assert.deepEqual(detailedPlanProjection(text), expected);
});
