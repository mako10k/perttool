...
test("unit migration fixes identity, directions, and effective velocity", async () => {
  ...
  assert.match(specification, /Document status: Normative 3\.0/);
  assert.match(
    specification,
...
test("unit migration fixes the complete source inventory and exact formulas", async () => {
  ...
  for (const field of [
    ...
    "every `work_event.planned_value`",
  ]) {
...
test("unit migration serializes every exact value and selects the grammar atomically", async () => {
  ...
  assert.match(
    specification,
    /Migration versions 2 and 3 have no\s+`nonrepresentable_decimal` failure/,
  );
  assert.match(
    specification,
  ...
  assert.match(
    specification,
    /retain source grammar version 3, 4, or 5 regardless of generated token\s+spelling/,
  );
  assert.match(
    specification,
...
test("unit migration fixes no-op, inverse, and existing-version boundaries", async () => {
  ...
  assert.match(
    specification,
    /inventory remains complete for\s+grammar versions 1, 2, 3, 4, and 5/,
  );
  assert.match(
    specification,
