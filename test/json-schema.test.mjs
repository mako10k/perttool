...
test("schema command lists, resolves, and rejects schema identities", () => {
  ...
  const text = run(["schema", "Perttool.CheckResult.v6"]).stdout;
  assert.match(text, /^Schema: Perttool\.CheckResult\.v6$/m);
  assert.match(
    text,
    /^Artifact: schemas\/Perttool\.CheckResult\.v6\.schema\.json$/m,
  );
...
test("actual success, invalid, unavailable, and usage results validate", (t) => {
  ...
  const results = [
    ...
    cliJson(["schema", "Perttool.CheckResult.v6"]),
