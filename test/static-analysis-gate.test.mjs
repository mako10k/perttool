...
test("source static analysis is pinned, ratcheted, documented, and required by CI", async () => {
  ...
  assert.equal(jscpd.threshold, 3.39);
  ...
  assert.equal(baseline.jscpd.maxDuplicatedLines, 2749);
  assert.equal(baseline.jscpd.maxDuplicatedTokens, 15252);
  assert.equal(baseline.jscpd.maxPercentage, 3.39);
  ...
  assert.equal(Object.keys(baseline.lizard.functions).length, 171);
