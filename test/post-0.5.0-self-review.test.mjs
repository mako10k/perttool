...
test("current guidance records the published 0.5.2 schema boundary", async () => {
  ...
  assert.match(
    agents,
    /`release-0\.2\.0\.pert` through `release-0\.7\.1\.pert` as independent post-beta workstreams/,
  );
  assert.doesNotMatch(
    agents,
  ...
  assert.match(schemaContract, /The first twenty identities are command results/);
