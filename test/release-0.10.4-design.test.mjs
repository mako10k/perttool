...
test("0.10.4 selects the compatible Issue 6 patch and retains public identities", async () => {
  ...
  assert.equal(JSON.parse(lspText).peerDependencies.perttool, "0.11.0");
  assert.equal(JSON.parse(mcpText).peerDependencies.perttool, "0.11.0");
  assert.match(versionSource, /TOOL_VERSION = "0\.11\.0"/u);
  assert.match(protocol, /MCP_SERVER_VERSION = "0\.11\.0"/u);
