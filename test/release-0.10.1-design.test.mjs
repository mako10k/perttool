...
test("0.10.1 selects the compatible Issue 22 patch and retains public identities", async () => {
  ...
  assert.equal(lsp.peerDependencies.perttool, "0.10.1");
  assert.equal(mcp.peerDependencies.perttool, "0.10.1");
  assert.match(versionSource, /TOOL_VERSION = "0\.10\.1"/u);
  assert.match(protocol, /MCP_SERVER_VERSION = "0\.10\.1"/u);
  assert.match(changelog, /^## \[0\.10\.1\] - 2026-08-20$/mu);
  assert.match(readme, /compatible `0\.10\.1`\s+Issue #22 migration-path patch/u);
