...
test("0.9.0 release and post-acceptance closure bind Grammar 7 and CLI Contract 8", async () => {
  ...
  assert.equal(lspManifest.peerDependencies.perttool, "0.10.2");
  assert.equal(mcpManifest.peerDependencies.perttool, "0.10.2");
  assert.match(versionSource, /TOOL_VERSION = "0\.10\.2"/u);
  assert.match(mcpProtocol, /MCP_SERVER_VERSION = "0\.10\.2"/u);
  ...
  assert.match(
    readme,
    /`latest=beta=0\.10\.0` and no `alpha`/u,
  );
