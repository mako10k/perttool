...
test("repository policy and self-use registration include the accepted workstream", async () => {
  ...
  for (const policy of [agents, copilot]) {
    ...
    assert.match(policy, /Perttool\.NextResult\.v8/);
    assert.match(policy, /`beta=0\.11\.0`, `latest=0\.10\.5`/);
    assert.match(policy, /0\.10\.6.*rollback pin/s);
  }
  ...
  assert.match(selfUseScript, /47 plans; check, analyze, next/);
  assert.match(selfUseGuide, /all forty-seven current plans/);
  ...
  assert.match(planIndex, /All forty-seven plans pass/);
