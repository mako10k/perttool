...
- ADR 0004 is active. Existing Japanese surfaces are migration debt in `plans/english-baseline.pert`. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, `PROCESS_AND_GUIDANCE_DOCS`, `RUNTIME_MESSAGES`, and `HELP_AND_USAGE` are complete and advanced; `PERT_PLANS` is complete at its exact pre-advance snapshot, and complete NextResult v5 now recommends only `GOLDEN_UNICODE_AUDIT`.
