...
- 現在はTypeScript CLIの操作系実装段階であり、実装済み`dsl check`/`dsl format`/`dsl help`/`dag analyze`/`dag next`/`dag render --to mermaid`、source-preserving formatter/application Core、task/milestone/resource mutation Core、atomic batch、safe-write I/O adapter、editing CLIの`--write`/`--out`/`--expect-digest`、grammar acceptance suiteでmacro `plans/mvp.pert`と詳細`plans/grammar.pert`、`plans/control-plane.pert`、`plans/operations.pert`をStage 2のpreview-first safe-writeで自己利用中。`dag advance`とMermaid importは未実装である。
