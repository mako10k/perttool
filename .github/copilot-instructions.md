...
- The first Contract 3 release target is suffix-free beta `0.2.0`, tracked by `plans/release-0.2.0.pert` and `docs/process/0.2.0-release.md`. `RELEASE_020_GATE_DESIGN` is complete and advanced; 14p remain at `3p/1d`, and `RELEASE_020_PREPARATION` is recommended. Package identity is still `0.1.0`. External distribution remains blocked pending explicit authorization for the named Git/GitHub/npm batch, and `latest` promotion remains separate.
