...
- The selected `plans/release-0.7.0.pert` workstream sequences six serial tasks
  ...
  instruction separately authorized source preparation; the current
  instruction separately authorizes candidate acceptance. The gate
  ...
  Contract 7 file-first and plan-assurance acceptance. Raw NextResult v6
  recommends only `RELEASE_070_PUBLISH`, which remains a separately authorized
  external batch, and `0.7.0` remains unpublished. Push, tag, GitHub/npm
