...
- The CLI/help review is tracked independently in `plans/cli-surface-reset.pert`. All nine tasks through `CLI_003_FILE_FIRST_ACCEPTANCE` are complete and advanced. The active source Contract 3 surface is registry-driven, separates command help, domain guide, and agent guidance, includes project initialization and gate maintenance, identifies all JSON envelopes as contract 3, and rejects renamed Contract 2 spellings. The isolated installed-package file-first E2E covers initialization, every entity field, analysis, selection, advance, and validation without manual rewriting. The plan has no remaining or recommended task at `49p/1d`; the published `0.1.0` package remains Contract 2, and acceptance does not authorize a release.
- The first Contract 3 release target is suffix-free beta `0.2.0`, tracked by `plans/release-0.2.0.pert` and `docs/process/0.2.0-release.md`. `RELEASE_020_GATE_DESIGN`, `RELEASE_020_PREPARATION`, and `RELEASE_020_CANDIDATE` are complete and advanced; the complete local gate passed, public identities are unused, and npm `beta=latest=0.1.0` is the verified baseline. 8p remain at `9p/2d`, and complete Next v3 recommends `RELEASE_020_DISTRIBUTION`. The user authorized the named Git force-update, annotated tag, GitHub prerelease, and npm `beta` batch; `latest` promotion remains separate.
