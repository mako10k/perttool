...
- 現在はdesign/specification段階であり、存在しないCLI、package script、testを推測しない。
...
- `docs/process/self-use.md`のgateを満たすまで、perttool自身を正本writerとして使用しない。
- repository checkは`bash scripts/check-docs.sh`、whitespace checkは`git diff --check`を使用する。
