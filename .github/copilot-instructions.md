...
- 現在はTypeScript CLIの操作系実装段階であり、実装済み`dsl check`/`dsl format`/`dsl help`/`dag analyze`/`dag next`/`dag render --to mermaid`、source-preserving formatter/application Core、task/milestone/resource mutation Core、atomic batch、safe-write I/O adapter、editing CLIの`--write`/`--out`/`--expect-digest`、canonical advance planner Core、grammar acceptance suiteでmacro `plans/mvp.pert`と詳細`plans/grammar.pert`、`plans/control-plane.pert`、`plans/operations.pert`をStage 2のpreview-first safe-writeで自己利用中。`dag advance` CLIとMermaid importは未実装である。
...
- `docs/process/self-use.md`のStage 2まで解禁済み。Editing writeはpreview、diff確認、expected digest、write後再解析を必須とし、`dag advance --write`はStage 3まで使用しない。
