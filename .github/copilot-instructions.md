...
- The TypeScript CLI MVP, recommendation MIG-01 through MIG-07, and the read-only AI Agent Guidance Registry v1 are accepted. The current source and accepted published `0.5.1` implement CLI Contract 6 with Grammar 1/2/3/4/5, owner-aware governed lifecycle, read-only history and velocity observation, exact unit migration, AnalysisResult v4, UnitMigrationResult v3, a complete known NextResult v5 as normal start authority, all seventeen active command-result Draft 2020-12 artifacts plus public OverrideDecision, an additive read-only `schema` command, and Git 2.54 UTC `Z` history compatibility. npm `beta=0.5.1`; npm `latest=0.4.0` remains CLI Contract 5 and Grammar 4, while pre-schema Contract 6 `0.5.0`, Contract 4 `0.3.0`, and Contract 3 `0.2.0` remain available by pin. Issue #5 closure remains a separate boundary. Issue #3, the LSP server, VSIX, and MCP server remain post-beta backlogs. Human override apply/audit remains unavailable until MIG-08.
