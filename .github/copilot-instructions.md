...
- The selected next release is suffix-free beta `0.5.0` for the atomic Grammar 5 and CLI Contract 6 cutover. `plans/release-0.5.0.pert` consumes accepted project-actuals and English-baseline input without duplicating either workstream. Git commit `1641a32` records the exact completed gate-design pre-advance snapshot, and that task is advanced. Git commit `ba84cd8` records the exact completed Contract 6 readiness pre-advance snapshot, and that task is advanced to reached `RELEASE_050_CONTRACT_6_READY`. `RELEASE_050_PREPARATION` is complete after aligning the `0.5.0` package identity, release guidance, tests, goldens, and full repository and installed-package gates; Git commit `e1e7ccf` records its exact pre-advance snapshot, and the task is advanced to reached `RELEASE_050_SOURCE_PREPARED`. `RELEASE_050_CANDIDATE` is complete and advanced after rechecking the clean source, channels, protected routes, full gates, and the retained 468-file tarball with SHA-256 `f3ba9b3f...2208c`. 6p remain, precedence and heuristic resource makespans are both 6p at observed `13p/1d`, both forecasts are `6/13d`, and complete NextResult v5 recommends only `RELEASE_050_PUBLISH`. The user's named authorization covers the gated release sequence and exact post-release local installation, but not npm `latest` promotion or Issue #4 closure.
