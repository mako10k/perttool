...
- The selected independent `plans/advance-clean-candidate.pert` workstream
  ...
  `ADV_CLEAN_CANDIDATE_CONTRACT` is complete and retained before advance. Its
  ...
  `docs/process/advance-clean-candidate-contract-acceptance.md`. Current
  runtime still has the eventful-advance trailing-blank-line defect, and
  `ADV_CLEAN_CANDIDATE_CORE` is the sole ready, recommended, and startable
  task. Do not globally trim or invoke the formatter. Implementation, plan
  advance, release, remote writes, publication, and dist-tag movement remain
  separate.
