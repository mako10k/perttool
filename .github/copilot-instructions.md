...
- ADR 0004 is active. Existing Japanese surfaces are migration debt in `plans/english-baseline.pert`. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, and `PROCESS_AND_GUIDANCE_DOCS` are complete and advanced, and `RUNTIME_MESSAGES` is the current recommended detail task.
