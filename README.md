...
現在は[MVPからbetaへのmacro計画](plans/mvp.pert)をroadmapとするStage 3のpreview-first自己利用を行っています。MVP public alphaは受け入れ済みで、Issue #2のread-only AI Agent Guidance Registryをbeta gateへ追加しました。[5 provider baseline](docs/process/agent-guidance-provider-baseline.md)、[公開contract](docs/specs/agent-guidance.md)、version付きoffline profile、validator、query、index/quick/detail projection、deterministic JSON/text、structured command help、`agent help` CLI、package-installed Core/CLI parityを完了し、固有実測Velocityを`19p/1d`へ補正しました。利用者の明示指示で`PROJECT_METADATA_CLI` 5pを先行完了し、操作系実測Velocityも累計29p/2 active dayの`29p/2d`へ再calibrationしました。Macroのrecommended work packageは再び`AGENT_GUIDANCE_IMPLEMENTATION`、[詳細plan](plans/agent-guidance.pert)のrecommended taskは`GUIDANCE_ACCEPTANCE`です。詳細残り3pのforecastは`3/19d`、macro roll-upは0.157895d、beta release E2Eを含むresource makespanは1.157895dです。Issue #3、LSP server、VSIX、MCP serverはbeta blockerに含めません。
