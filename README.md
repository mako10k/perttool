...
現在は[MVPマイルストーン計画](plans/mvp.pert)をmacro roadmap、[文法作業計画](plans/grammar.pert)、[AI工程制御設計計画](plans/control-plane.pert)、[操作系M1-M4実装計画](plans/operations.pert)を詳細planとするStage 2のpreview-first safe-write自己利用を行っています。Mermaid exportとadvance planner Coreまで完了し、macroの残るprecedence makespanは7d、resource makespanは7.090909dです。`ADVANCE`と`MERMAID_ROUNDTRIP`がreadyですが、現行schedulerはpriorityと`REVIEWERS` capacityから`ADVANCE`だけを`runnable_now`にします。対応する次の詳細taskは`ADVANCE_CLI_ACCEPTANCE`です。操作系Velocityは実測`22p/1d`で、day見積りのMermaid macro taskはこのPoint標本へ混ぜません。RecommendationとIssue #2のAI Agent Guidance RegistryはM3後に詳細化可能ですが、macro planへ追加するまでは着手順を推測しません。Issue #3のmulti-plan compositionはMVP後の将来構想です。
