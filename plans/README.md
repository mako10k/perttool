...
The `release-0.2.0.pert` gate selects suffix-free beta `0.2.0` for the first Contract 3 package. `RELEASE_020_GATE_DESIGN` completed and advanced on 2026-07-24, establishing a first plan-specific observation of `3p/1d`. Four tasks and 14p remain; precedence and heuristic resource makespans are both 14p with no resource delay and are conditional on authorization for `RELEASE_020_DISTRIBUTION`. Complete Next v3 recommends `RELEASE_020_PREPARATION`. Package identity remains `0.1.0`, and no Git, GitHub, npm, or dist-tag write is authorized by this plan state.
