...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. `TEMPORAL_REQUIREMENTS` fixed the accepted property scope, `CALENDAR_SEMANTICS` fixed exact date/date-time comparison, `as_of`, fixed-offset projection, point velocity, `not_before`, and the continuous-calendar boundary, and `DEADLINE_SEMANTICS` fixed task, milestone, and project deadline evaluation, signed margins, precedence lower bounds, heuristic resource forecasts, combined assessment, blocked-cone qualification, and explicit unavailable causes. The first two tasks are complete and advanced; `DEADLINE_SEMANTICS` is complete and retained in the committed pre-advance snapshot. Four unfinished tasks and 13p remain in the SU-M1 detail, with 13p precedence and heuristic resource makespans and no resource delay. Its cumulative provisional velocity is `11p/1d`, giving exact precedence/resource forecasts of `13/11d`; complete `NextResult.v3` recommends `UNIT_MIGRATION_SEMANTICS`. Rolling the resource forecast to six decimal places into `SU_M1_CONTRACT_WORK_PACKAGE` gives the macro provisional `7.181818d` precedence and `9.181818d` heuristic resource makespans with 2d resource delay. The macro continues to recommend `SU_M1_CONTRACT_WORK_PACKAGE`; later work-package estimates are re-baselined when the preceding milestone is accepted.
