...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent scheduling-and-units roadmap. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the finite-decimal migration limit, the user selected an exact fraction Duration extension as common milestone SU-M2R before SU-M3 and SU-M4. `RATIONAL_DURATION_CONTRACT` is committed and advanced. The separate Grammar 3 source model and exact Duration serializer are implemented and done but unadvanced under ADV-001 until their implementation snapshot is committed. Thirteen points remain with 7p precedence and 10p heuristic resource makespans, 3p delay, and inherited provisional forecasts of `7/24d` and `5/12d`. The macro rollup is `0.416667d`, leaving `4.416667d` precedence and `6.416667d` resource makespans with 2d delay. Complete Next v3 recommends `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE` in the macro and the jointly feasible `RATIONAL_DURATION_FORMATTER` plus `RATIONAL_DURATION_MUTATION` in the detail; `RATIONAL_DURATION_VERSION_BOUNDARY` is deferred by developer capacity. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public schemas, help, installed workflows, and publication remain unchanged.
