...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. `TEMPORAL_REQUIREMENTS` fixed the accepted property scope, and `CALENDAR_SEMANTICS` then fixed exact date/date-time comparison, `as_of`, fixed-offset projection, point velocity, `not_before`, and the continuous-calendar boundary. Both tasks are complete and advanced. The SU-M1 detail now contains five tasks totaling 17p, with 13p precedence and 17p heuristic resource makespans and 4p resource delay. Its cumulative provisional velocity is `7p/1d`, giving exact precedence/resource forecasts of `13/7d` and `17/7d`; complete `NextResult.v3` recommends `DEADLINE_SEMANTICS`. Rolling the resource forecast to six decimal places into `SU_M1_CONTRACT_WORK_PACKAGE` gives the macro provisional `8.428571d` precedence and `10.428571d` heuristic resource makespans with 2d resource delay. The macro continues to recommend `SU_M1_CONTRACT_WORK_PACKAGE`; later work-package estimates are re-baselined when the preceding milestone is accepted.
