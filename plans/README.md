...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. All seven SU-M1 tasks through `M1_CONTRACT_REVIEW` are complete and advanced, and the [acceptance review](../docs/process/scheduling-units-m1-acceptance.md) traces all `TUI-001` through `TUI-018` observations. The review fixes SU-M2 through SU-M4 as target-only source/Core slices and SU-M5 as the atomic Contract 4 and Next v4 authority cutover. The completed SU-M1 detail has no recommendation at a cumulative provisional `24p/1d`. In the current SU-M2 detail, `GRAMMAR_V2_TEMPORAL_SOURCE` and `TEMPORAL_SEMANTIC_VALIDATION` are complete and advanced, and `TEMPORAL_FORMATTER_ROUNDTRIP` is complete without activating Grammar 2 or Contract 4 publicly. Three tasks and 12p remain; the precedence makespan is 9p and the heuristic resource makespan is 12p with 3p resource delay. Observed provisional velocity is `12p/1d`, the forecasts are `3/4d` and `1d`, and complete `NextResult.v3` recommends `TEMPORAL_MUTATION_BATCH_CORE`; `DECLARED_TEMPORAL_INPUT_CORE` is deferred by the selected task's `TARGET_CORE_SURFACE` use. The resource forecast is rolled up to the macro, whose provisional precedence and heuristic resource makespans are now `5d` and `7d` with 2d resource delay; its recommendation remains `SU_M2_TEMPORAL_SURFACE_WORK_PACKAGE`.
