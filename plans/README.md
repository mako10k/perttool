...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. `TEMPORAL_REQUIREMENTS` fixed the accepted property scope, `CALENDAR_SEMANTICS` fixed exact date/date-time comparison, `as_of`, fixed-offset projection, point velocity, `not_before`, and the continuous-calendar boundary, `DEADLINE_SEMANTICS` fixed task, milestone, and project deadline evaluation, `UNIT_MIGRATION_SEMANTICS` fixed Point-to-linked-time directions, velocity disposition, complete source-field inventory, exact finite-decimal conversion, atomic candidates, and inverse qualification, `INTERFACE_PROJECTION_CONTRACT` selected target Grammar 2, CLI Contract 4, temporal/unit result identities, mutation/help/diagnostics, and release-gated start authority, and `NORMATIVE_EXAMPLES` fixed `TUE-001` through `TUE-018` source and machine-readable witnesses. All six tasks are complete and advanced. One task and 2p remain in the SU-M1 detail, with 2p precedence and heuristic resource makespans and no resource delay. Its cumulative provisional velocity is `22p/1d`, giving exact precedence/resource forecasts of `1/11d`; complete `NextResult.v3` recommends `M1_CONTRACT_REVIEW`. Rolling the resource forecast into `SU_M1_CONTRACT_WORK_PACKAGE` gives the macro provisional `6.090909d` precedence and `8.090909d` heuristic resource makespans with 2d resource delay. The macro continues to recommend `SU_M1_CONTRACT_WORK_PACKAGE`; later work-package estimates are re-baselined when the preceding milestone is accepted.
