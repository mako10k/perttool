...
- [scheduling-units-m2.pert](scheduling-units-m2.pert): completed target-only Grammar 2 temporal source and Core-foundation detail through SU-M2, pending canonical advance
...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. SU-M1 is complete and advanced at `24p/1d`. All six SU-M2 tasks are accepted at `24p/1d`; the [SU-M2 acceptance record](../docs/process/scheduling-units-m2-acceptance.md) traces the target Core evidence and keeps Grammar 2, Contract 4, target schemas, help, Guide, installed workflows, and publication inactive. The completed detail and macro work package await their committed-snapshot advance. The remaining macro has 4d precedence and 6d heuristic resource makespans with 2d delay. Both SU-M3 and SU-M4 are ready: the scheduler set contains SU-M3, while complete Next v3 recommends the different jointly feasible SU-M4 set, as permitted by the accepted recommendation contract.
