...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent scheduling-and-units roadmap. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the finite-decimal migration limit, the user selected an exact fraction Duration extension as common milestone SU-M2R before SU-M3 and SU-M4. Contract, Grammar 3 source-model, exact-serializer, formatter, project/task/batch mutation, and the internal grammar version boundary are committed and advanced; the formatter/mutation implementation is fixed at `f0d9a26` and the version boundary at `fa698ca`. Final acceptance is implemented and done but unadvanced until its snapshot is committed under ADV-001. The [acceptance record](../docs/process/scheduling-units-m2r-acceptance.md) traces TUI/TUE through 020, fixes exact TUE-015 representation without claiming the later migration command, and verifies internal composition, closed root exports, and installed Contract 3 rejection of Grammar 2/3. All 24p are done at provisional `24p/1d`; the detail has zero makespans and no recommendation at digest `sha256:ccb862377a719215e551f2f9f0226745c3d6c3cc3b0b5736d1875dc1826d6d6e`. Until committed Advance and one macro rollup, the macro remains `0.166667d`, leaving `4.166667d` precedence and `6.166667d` resource makespans with 2d delay and a recommendation for `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE`. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public schemas, help, installed workflows, Next v3 authority, and publication remain unchanged.
