...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent `SU-M0` through `SU-M5` milestone roadmap. `TEMPORAL_REQUIREMENTS` fixed the accepted property scope, `CALENDAR_SEMANTICS` fixed exact date/date-time comparison, `as_of`, fixed-offset projection, point velocity, `not_before`, and the continuous-calendar boundary, `DEADLINE_SEMANTICS` fixed task, milestone, and project deadline evaluation, and `UNIT_MIGRATION_SEMANTICS` fixed Point-to-linked-time directions, velocity disposition, complete source-field inventory, exact finite-decimal conversion, atomic candidates, and inverse qualification. The first three tasks are complete and advanced; `UNIT_MIGRATION_SEMANTICS` is complete and retained in the committed pre-advance snapshot. Three unfinished tasks and 9p remain in the SU-M1 detail, with 9p precedence and heuristic resource makespans and no resource delay. Its cumulative provisional velocity is `15p/1d`, giving exact precedence/resource forecasts of `3/5d`; complete `NextResult.v3` recommends `INTERFACE_PROJECTION_CONTRACT`. Rolling the resource forecast into `SU_M1_CONTRACT_WORK_PACKAGE` gives the macro provisional `6.6d` precedence and `8.6d` heuristic resource makespans with 2d resource delay. The macro continues to recommend `SU_M1_CONTRACT_WORK_PACKAGE`; later work-package estimates are re-baselined when the preceding milestone is accepted.
