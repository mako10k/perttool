...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent scheduling-and-units roadmap. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the finite-decimal migration limit, the user selected an exact fraction Duration extension as common milestone SU-M2R before SU-M3 and SU-M4. Contract, Grammar 3 source-model, exact-serializer, formatter, and project/task/batch mutation tasks are committed and advanced; the formatter/mutation implementation is fixed at `f0d9a26`. The internal grammar version boundary is implemented and done but unadvanced under ADV-001 pending its implementation commit. Four points remain with equal 4p precedence/resource makespans, no delay, and inherited provisional forecasts of `1/6d`. The macro rollup is `0.166667d`, leaving `4.166667d` precedence and `6.166667d` resource makespans with 2d delay. Complete Next v3 recommends `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE` in the macro and `RATIONAL_DURATION_ACCEPTANCE` in the detail. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public schemas, help, installed workflows, and publication remain unchanged.
