...
On 2026-07-25, `TIME-001` and `UNIT-001` were refined into the independent scheduling-and-units roadmap. SU-M1, SU-M2, and the exact fraction Duration extension SU-M2R are complete and advanced at `24p/1d` each. SU-M2R was rolled up at `a93c129` and the macro frontier advanced at `b4e891d`. Accepted semantics re-estimate SU-M3 at 23p and SU-M4 at 25p; complete Next v3 selects SU-M4. The current `scheduling-units-m4.pert` contains six tasks totaling 25p, with 21p precedence, 25p heuristic resource work, 4p delay, forecasts `7/8d` and `25/24d`, and a recommendation for `MIGRATION_REQUEST_AND_INVENTORY`. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public schemas, help, installed workflows, Next v3 authority, and publication remain unchanged.
