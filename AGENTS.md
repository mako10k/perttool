...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and the SU-M1/SU-M2 details. SU-M0 through SU-M5 separate backlog refinement, contract acceptance, temporal source/Core foundations, temporal deadline and Next v4 target Core, exact unit-migration Core, and the atomic Contract 4 cutover. The accepted initial temporal scope is project `as_of`, milestone `deadline`, task `not_before`, and task `deadline`; the accepted Calendar, Deadline, Migration, and Interface contracts retain their version 1 identities and target Grammar 2/CLI Contract 4/result identities without activating them in Contract 3. `TUE-001` through `TUE-018` remain the normative target observations. The SU-M1 detail is complete and advanced at `24p/1d`. All six SU-M2 tasks through `M2_FOUNDATION_ACCEPTANCE` are accepted at `24p/1d`; the cross-cutting record is `docs/process/scheduling-units-m2-acceptance.md`, and the completed detail and macro work package await their committed-snapshot advance. Target parsing, validation, formatting, temporal mutation/batch/safe write, and CheckResult v2/ProjectResult v2 declared-input Core are internal. Active Grammar 1, CLI Contract 3, public result schemas, root exports, the 27-command registry, help, Guide, and installed behavior remain unchanged and reject Grammar 2. The remaining macro has 4d precedence and 6d heuristic resource makespans with 2d delay. Its scheduler set is `SU_M3_DEADLINE_CAPABILITY_WORK_PACKAGE`, while the complete Next v3 recommendation is the different jointly feasible set `SU_M4_UNIT_MIGRATION_WORK_PACKAGE`, as permitted by Recommendation section 10; do not substitute `runnable_now` for recommendation authority. Later work-package estimates remain provisional until their preceding milestone is accepted.
