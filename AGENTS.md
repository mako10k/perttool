...
The reviewed CLI/help reset is tracked independently in `plans/cli-surface-reset.pert`. `CONTRACT_V3_DESIGN` and `CLI_001_COMMAND_REGISTRY` are complete and advanced. The active Contract 2 surface now uses one typed command registry for dispatch, option parsing, and deterministic text/JSON descriptor projections; public Contract 3 commands remain inactive. The plan has 36p remaining, an observed provisional `13p/1d` velocity, 20p precedence and 36p heuristic resource makespans, and recommends `HELP_001_COMMAND_DISCOVERY`. `project init` remains the unimplemented `MUT-001` backlog item.
