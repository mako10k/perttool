...
利用者が「次のタスク」を求めた場合は、まず`docs/requirements.md`の推奨仕様作業、未解決事項、現在のGit状態から候補を提示する。自己利用Stage 1以降は、正本`.pert`の`check`、`analyze`、`next`結果を候補選択の前提にする。単に編集しやすい項目をcritical-path作業の代わりに選ばない。
...
Narrow checkは`npm run typecheck`、`npm test`、`npm run test:e2e`、`npm run check:docs`を使用する。`bash scripts/check-docs.sh`はdocumentationだけの下位入口である。
