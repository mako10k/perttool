...
ADR 0004 adopts English as the repository baseline. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, `PROCESS_AND_GUIDANCE_DOCS`, `RUNTIME_MESSAGES`, `HELP_AND_USAGE`, `PERT_PLANS`, and `GOLDEN_UNICODE_AUDIT` are complete and advanced; `9d9fdbf` and `81ab774` record the exact completed PERT-plan and golden/Unicode pre-advance snapshots. `plans/english-baseline.pert` now has 3p remaining, matching 3p precedence and resource makespans, an observed `39p/2d` velocity, and recommends `ENGLISH_ACCEPTANCE`.
