...
- `src/migration/`: internal exact unit-migration request validation, velocity selection, stable causes, complete Duration inventory, preserved-temporal snapshots, and exact-Duration grammar selection, compatibility, reversibility, and localized version-upgrade inputs.
...
- `src/application/`: pure services for active check/project initialization/project metadata/analyze/next, target declared-input check/project projections, capability-checked unit-migration request preparation, separate active/Grammar 2/Grammar 3 mutation planning, and the internal exact-Duration version-candidate boundary.
