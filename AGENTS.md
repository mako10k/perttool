...
artifact, installed replays, and Issue completion agree. Version `0.9.4` is the durably accepted compatible Issue #19
patch. Its exact release commit, tag, CI, GitHub prerelease, npm artifact,
installed replay, and Issue completion agree; npm `latest` remains separate.
The
...
`sha256:fac511d0...87af00`. The current private implementation composes the
existing Grammar 6/7 unit migration, complete unsealed assurance proof,
protected-record classification, exact recovery, and current document binding
through one pure evaluator and Application facade. Model 2 exposes only the
accepted preferred Quick Fix and atomic `source.fixAll.perttool`; model 1,
E0 formatting, all public counts, and direct-write ownership remain unchanged.
...
release corrected `ADV-006`, closed Issue #19 with release evidence, and left
no open P0. Issues #7 / `ACT-004` and #6 / `ACT-005` are P1. Issues #13, #18,
and #12 are P2, and Issue #3 is P3. The selected editor plan therefore resumed
from its independently retained `EDITOR_REPAIR_ACCEPTANCE` authority. The
published `0.9.4` and exact rollback evidence are in
`docs/process/0.9.4-release-acceptance.md`; no later Issue mutation or another
release is implied by the editor implementation.
...
The explicitly selected compatible `0.9.2` Point-without-velocity emergency
patch is tracked in `plans/release-0.9.2.pert`. `RELEASE_092_SELF_REVIEW` and
`RELEASE_092_CORRECTION` are complete after reproducing Issue #15 in the
active local `0.9.1` installation and exact peeled `v0.9.1` source, then
accepting the narrow temporal-layer correction. Valid Point plans without a
declared velocity retain Point-valued precedence, resource, recommendation,
and start-authority facts, with nullable forecasts; temporal conversion that
actually needs a rate reports `missing_velocity`. Grammar 7, CLI Contract 8,
53 commands, 23 root schemas, 129 root and Node exports, 45 Core exports,
result and schema identities, validation, recommendation, governance, and
mutation authority remain unchanged. The isolated worktree is based on exact
commit `ddb12dc97e8be34965654c84f01bc330a4a2e5cf`.
`RELEASE_092_PREPARATION` is complete after aligning every version-bearing
surface and passing the complete Node.js 22 gate: 1,048 tests, 40 self-use
plans, documentation and English checks, private adapters, temporary link,
and the 712-file isolated package. Its record is
...
startable only `RELEASE_092_PUBLISH` before publication. Release commit and
peeled annotated tag `v0.9.2` target `b47a0a4`; Node.js 22 and 24 CI run
`31762661717` passed on its single bounded failed-job rerun after the first
attempt timed out in the unchanged external VS Code host probe. GitHub
prerelease `370310054`, npm `beta=0.9.2`, and the retained candidate are
byte-identical at SHA-256 `5347f4b0...bec275f`. npm accepted exactly one
publish; the short script readback returned propagation E404, the operation
was not retried, and fresh reads established durable publication at
`2026-08-14T02:16:08.927Z`. Exact and beta installations pass the Issue #15
matrix, and exact `0.9.1` remains installable as rollback. Active Node.js 22
and 25 global installations now resolve to `0.9.2`. Issue #15 is closed as
completed with one evidence comment and only `bug` retained. All six tasks
and 18p are complete with zero makespans and no ready, recommended, or
startable task; durable acceptance is recorded in
...
The compatible `0.9.4` emergency patch is tracked independently in
`plans/release-0.9.4.pert`. Its six serial tasks total 15p from
`RELEASE_094_SELF_REVIEW` through `RELEASE_094_ACCEPTANCE`. The accepted source retains criterion sets
and receipts for every milestone kept by `dag advance`, contracts only records
owned by removed milestones, checks the complete final Contract 8 candidate,
and merges its diagnostics deterministically. The release source also adds
pinned jscpd 5.0.15 duplicate and Lizard 1.23.0 complexity ratchets to
`check:static` and CI without changing Grammar 7, CLI Contract 8, the 53
commands, 23 root schemas, 129 root and Node exports, 45 Core exports, public
authority, or force semantics. Release commit and peeled annotated `v0.9.4`
target `094132c`; Node.js 22 and 24 CI run `31786430400` passed; the retained,
GitHub, and npm 712-file tarballs agree at SHA-256
...
- `src/command/`: immutable typed Contract 10 command descriptors, shared-option expansion, dispatch lookup, deterministic text/JSON help projections, and structured usage-error recovery.
- `src/core/`: additive platform-neutral `perttool/core` entrypoint and the portable Grammar 1 through 9 parse/validate/format and planning facade; its current exact 51-name runtime catalog has no Node, Application, CLI, I/O, history, schema-loader, or adapter dependency.
...
- `src/help/`: the structured domain HelpNode registry, retained Core help data, and the active Contract 10 editing, actuals, plan-assurance, historical-DAG, milestone-acceptance, temporal-schedule, and Planning Pool Guide.
...
- `src/schema/`: closed Contract 10 result-schema catalog, local bundled-artifact resolution, `Perttool.SchemaResult.v1`, and deterministic JSON/text projections.
...
- `schemas/`: twenty-nine bundled Draft 2020-12 active root artifacts for every command result and public library-only result, plus shared local definitions.
...
- `src/node/`: additive `perttool/node` entrypoint, default public Node Host composition, and private historical and Planning Pool Git-evidence CLI composition; root and Node expose the same 139 runtime values with exact key and reference identity while Core remains portable.
...
- `src/application/`: exact compatibility re-exports for neutral check, analyze, mutation, target-mutation, temporal, and Planning Pool implementations; the private CLI Application facade with injected Node Host ports; active Contract 10 Grammar 9 composition with `CheckResult.v6`, `AnalysisResult.v7`, `NextResult.v8`, `MutationResult.v6`, and `AdvanceResult.v4`; project metadata, lifecycle, project history, historical DAG, Planning Pool observation and history, exact unit migration, assurance mutation and inspection, independent acceptance/assurance/history guards, and authorization-before-safe-write orchestration.
...
For normal task selection, use only a known `Perttool.NextResult.v8`, recommendation interface 1, ranking algorithm 1, reason taxonomy 1.0, explanation/expression/description model 1, locale `en`, authority policy `recommendation_v1_plus_release_gate_plus_plan_assurance_v1`, complete assurance authority, complete milestone-acceptance projection, and a complete, non-truncated trace. Start only tasks listed in `startable_recommended_task_ids`, and require each ID to also be present in `groups.runnable_now`. A raw recommended ID outside `runnable_now` is informational and non-executable in that result. You may choose a subset of the startable set, or retain the complete set with exactly one additional resource-feasible, time-eligible, assurance-eligible `allowed` task. Do not start for an unknown version or authority policy, incomplete trace, `PTREC-*`, assurance safe stop, non-runnable raw recommendation, future or unavailable release eligibility, withheld assurance eligibility, unavailable milestone acceptance, or a `deferred`/`discouraged` selection; stop safely. Reanalyze rather than reusing the same result after task start, completion, blocking, time, capacity, plan, relation, outcome, assurance, or acceptance changes. Do not apply a selection requiring a human override until MIG-08; report its difference from the normal recommendation and the still-unavailable audit/apply boundary.
