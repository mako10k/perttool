...
ADR 0004 adopts English as the repository baseline. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, and `PROCESS_AND_GUIDANCE_DOCS` are complete and advanced; `plans/english-baseline.pert` now has 19p remaining, an 11p precedence makespan, a 16p resource makespan, and an observed `23p/1d` velocity, and recommends `RUNTIME_MESSAGES`.
