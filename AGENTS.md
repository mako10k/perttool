...
ADR 0004 adopts English as the repository baseline. `SURFACE_INVENTORY` and `NORMATIVE_DOCS` are complete and advanced; `plans/english-baseline.pert` now has 27p remaining with an observed `15p/1d` velocity and recommends `PROCESS_AND_GUIDANCE_DOCS`.
