...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and its milestone details. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the accepted finite-decimal migration limit, the user selected an exact rational Duration extension as new common milestone SU-M2R before SU-M3 and SU-M4. `RATIONAL_DURATION_CONTRACT`, `RATIONAL_DURATION_SOURCE_MODEL`, and `EXACT_DURATION_SERIALIZER` are committed and advanced. `RATIONAL_DURATION_FORMATTER` and `RATIONAL_DURATION_MUTATION` are implemented and done but unadvanced until their implementation snapshot is committed under ADV-001. Separate internal Grammar 3 formatting canonicalizes exact Duration without changing semantic values; project/task/batch mutation canonicalizes only changed Duration fields, validates one final candidate, rejects invented migration members, and reuses safe writes. Seven points remain; precedence and heuristic resource makespans are both 7p with no delay, and both inherited forecasts are `7/24d`. The macro rollup is `0.291667d`, leaving `4.291667d` precedence and `6.291667d` heuristic resource makespans with 2d delay. Complete Next v3 recommends `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE` in the macro and `RATIONAL_DURATION_VERSION_BOUNDARY` in the detail. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public result schemas, root exports, the 27-command registry, help, Guide, and installed behavior remain unchanged. Public Contract 4 activation, unit-migration command publication, Next v4 authority, release, and external distribution remain later gates.
...
- `src/application/`: pure services for active check/project initialization/project metadata/analyze/next, target declared-input check/project projections, and separate active/Grammar 2/Grammar 3 mutation planning that return validated documents or revalidated candidates.
