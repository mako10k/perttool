...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and its milestone details. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the accepted finite-decimal migration limit, the user selected an exact rational Duration extension as new common milestone SU-M2R before SU-M3 and SU-M4. The contract, source-model, serializer, formatter, mutation, and version-boundary tasks are committed and advanced. `RATIONAL_DURATION_ACCEPTANCE` is implemented and done but unadvanced until its acceptance snapshot is committed under ADV-001. The [SU-M2R acceptance record](docs/process/scheduling-units-m2r-acceptance.md) traces TUI/TUE through 020, fixes exact TUE-015 representation without claiming the later migration command, and verifies internal composition, closed root exports, the unchanged 27-command registry, and installed Contract 3 rejection of Grammar 2/3. All 24p are done at provisional `24p/1d`; the detail has zero precedence and heuristic resource makespans and no recommendation, with digest `sha256:ccb862377a719215e551f2f9f0226745c3d6c3cc3b0b5736d1875dc1826d6d6e`. Until the committed detail is advanced and rolled up once, the macro remains at `0.166667d`, with `4.166667d` precedence and `6.166667d` heuristic resource makespans, 2d delay, and a recommendation for `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE`. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public result schemas, root exports, help, Guide, Next v3 authority, and publication remain unchanged. SU-M3 owns temporal analysis, SU-M4 owns unit-migration orchestration and UnitMigrationResult v2, and SU-M5 owns public Contract 4 activation.
...
- `src/migration/`: internal exact-Duration grammar selection, compatibility qualification, reversibility, and localized version-upgrade planning inputs for the later unit-migration Core.
- `src/model/`: shared syntax/CST records, diagnostics, exact Rational arithmetic, units, internal declared calendar values, additive exact Duration Fraction values, and exact Decimal-or-Fraction source serialization.
...
- `src/application/`: pure services for active check/project initialization/project metadata/analyze/next, target declared-input check/project projections, separate active/Grammar 2/Grammar 3 mutation planning, and the internal exact-Duration version-candidate boundary.
...
Run the repository checks from the root with Node.js 22 or later. CI verifies Node.js 22 and 24. `npm run check` includes check/analyze/next validation for all thirteen self-use plans, including the scheduling-and-units macro, completed SU-M1/SU-M2 details, and current SU-M2R detail.
