...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and its milestone details. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the accepted finite-decimal migration limit, the user selected an exact rational Duration extension as new common milestone SU-M2R before SU-M3 and SU-M4. `RATIONAL_DURATION_CONTRACT` is committed and advanced. `RATIONAL_DURATION_SOURCE_MODEL` and `EXACT_DURATION_SERIALIZER` are implemented, reviewed, and done but remain unadvanced until their implementation snapshot is committed under ADV-001. A separate identity-checked Grammar 3 parser/validator retains reduced exact Fraction values and source spans; the internal serializer emits the shortest exact Decimal or a reduced Fraction without display rounding. Thirteen points remain with 7p precedence, a 10p heuristic resource makespan, 3p delay, and inherited provisional forecasts `7/24d` and `5/12d`. The macro rollup is `0.416667d`, leaving `4.416667d` precedence and `6.416667d` heuristic resource makespans with 2d delay. Complete Next v3 recommends `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE` in the macro and the jointly feasible `RATIONAL_DURATION_FORMATTER` plus `RATIONAL_DURATION_MUTATION` in the detail; `RATIONAL_DURATION_VERSION_BOUNDARY` is deferred by developer capacity. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public result schemas, root exports, the 27-command registry, help, Guide, and installed behavior remain unchanged. Public Contract 4 activation, unit-migration command publication, Next v4 authority, release, and external distribution remain later gates.
