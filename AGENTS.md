...
ADR 0004 adopts English as the repository baseline. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, `PROCESS_AND_GUIDANCE_DOCS`, `RUNTIME_MESSAGES`, `HELP_AND_USAGE`, and `PERT_PLANS` are complete and advanced; `9d9fdbf` records the exact completed PERT-plan pre-advance snapshot. `plans/english-baseline.pert` now has 6p remaining, matching 6p precedence and resource makespans, an observed `36p/2d` velocity, and recommends `GOLDEN_UNICODE_AUDIT`.
