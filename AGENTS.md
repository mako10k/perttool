...
The published release awaiting acceptance is suffix-free beta `0.4.0` for the breaking
...
passed. Three points remain, both makespans are 3p, observed cumulative
velocity is `16p/2d`, and the forecast is `3/8d`. Complete NextResult v4
recommends and starts only `RELEASE_040_ACCEPTANCE`. npm `latest` promotion
and Issue #4 closure remain separate post-acceptance decisions.
