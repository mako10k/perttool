...
Issue #4 is tracked in the independent `plans/governance.pert` post-beta workstream. `GOV_REQUIREMENTS`, `GOV_AUTHORITY_CONTRACT`, `GOV_DSL_CONTRACT`, `GOV_INTERFACE_CONTRACT`, `GOV_NORMATIVE_EXAMPLES`, `GOV_DESIGN_ACCEPTANCE`, `GOV_SOURCE_MODEL`, `GOV_AUTHORITY_CORE`, `GOV_CLI_PREVIEW`, and `GOV_WRITE_ENFORCEMENT` are complete and advanced. Internal target Grammar 4 parsing, declared/effective metadata and digest-bound snapshots, formatting, project init/show/set and batch fields, unit-migration preservation, deterministic actual-change classification, caller-assertion normalization, pre-change authority decisions, PTGOV diagnostics, governed direct/batch/advance previews, ProjectResult v3, MutationResult v2, GovernanceDecision v1, the complete Contract 5 registry/help/usage projection, and guarded in-place/existing-document-out persistence are implemented. Preview, denied, invalid, and stale decisions fail closed before or within the retained safe-write gates. These target capabilities remain absent from the active Contract 4 package root and CLI; guidance and atomic Contract 5 activation are not implemented. Complete NextResult v4 recommends and starts only `GOV_GUIDANCE`. The plan has 7p remaining, matching 7p precedence and heuristic resource makespans with no resource delay, and an observed velocity of `38p/2d`. Issue #3, the LSP server, VSIX, and MCP server remain post-beta backlogs. Human override apply, durable audit, and Git integration remain unavailable until MIG-08.
