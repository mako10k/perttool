...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and its milestone details. SU-M1, SU-M2, SU-M2R, and SU-M4 are complete and advanced. SU-M4's final acceptance snapshot is committed at `bc75b37`; all six detail tasks and 25p are accepted at `25p/1d`, and its macro package was rolled up once before advance. SU-M3 is the only ready macro package. Its acceptance snapshot is committed at `9c61bac`; all six detail tasks and 23p are accepted at `23p/1d`, and the detail is advanced to reached `DEADLINE_CAPABILITIES_ACCEPTED` with zero makespans and no recommendation. The acceptance record traces TUI/TUE through 020 and verifies exact calendar input, release-aware precedence and heuristic resource scheduling, deadline evaluation, internal AnalysisResult v3/NextResult v4 composition, deterministic failure behavior, and the closed Contract 3/package boundary with no open findings. The macro package remains planned until one rollup, after which only SU-M5 remains. Active Grammar 1, CLI Contract 3, public schemas, root exports, help, Guide, installed behavior, Next v3 normal authority, and publication remain unchanged until the atomic SU-M5 cutover.
