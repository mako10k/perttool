...
491-file, 520876-byte tarball. `RELEASE_053_PUBLISH` is the only current
startable recommendation. npm `latest` promotion remains a separate decision.
...
Run the repository checks from the root with Node.js 22 or later. CI verifies Node.js 22 and 24. `npm run check` includes check/analyze/next validation for all twenty-four self-use plans, including the active `0.5.3` and completed `0.5.2`, `0.5.1`, and `0.5.0` release plans, project-actuals, owner-aware governance, scheduling-and-units macro, SU-M1/SU-M2/SU-M2R/SU-M3/SU-M4/SU-M5 detail, `0.3.0`, and `0.4.0` plans.
