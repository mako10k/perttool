...
Run the repository checks from the root with Node.js 22 or later. CI verifies Node.js 22 and 24. `npm run check` includes check/analyze/next validation for all eleven self-use plans, including the scheduling-and-units macro and completed SU-M1 detail.
