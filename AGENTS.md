...
The reviewed CLI/help reset is tracked independently in `plans/cli-surface-reset.pert`. `CONTRACT_V3_DESIGN`, `CLI_001_COMMAND_REGISTRY`, `HELP_001_COMMAND_DISCOVERY`, and `MUT_002_GATE_MAINTENANCE` are complete and advanced. The active Contract 2 surface uses one typed command registry for dispatch and option parsing, while the internal Contract 3 preview provides deterministic top-level, resource, and action text/JSON projections including gate maintenance; public Contract 3 commands remain inactive. The plan has 23p remaining, an observed provisional `26p/1d` velocity, 15p precedence and 23p heuristic resource makespans, and recommends `MUT_001_PROJECT_INIT`. `project init` remains the unimplemented `MUT-001` backlog item.
