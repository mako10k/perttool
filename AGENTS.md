...
The reviewed CLI/help reset is tracked independently in `plans/cli-surface-reset.pert`. `CONTRACT_V3_DESIGN` is complete and advanced; Contract 3 is an accepted design target but remains unimplemented. The plan has 44p remaining, an observed provisional `5p/1d` velocity, 28p precedence and 44p heuristic resource makespans, and recommends `CLI_001_COMMAND_REGISTRY`. `project init` remains the unimplemented `MUT-001` backlog item.
