...
The explicitly selected `TIME-001` and `UNIT-001` workstream is tracked by the milestone-level `plans/scheduling-units.pert` and its milestone details. SU-M1 and SU-M2 are complete and advanced at `24p/1d` each. After reviewing the accepted finite-decimal migration limit, the user selected an exact rational Duration extension as new common milestone SU-M2R before SU-M3 and SU-M4. `RATIONAL_DURATION_CONTRACT` is done but unadvanced because the new detail has no committed pre-advance snapshot. It accepts Grammar 3, `perttool.temporal-unit-interface@2`, `perttool.unit-migration@2`, and `Perttool.UnitMigrationResult.v2`; exact Duration uses the shortest finite Decimal or a reduced Fraction, and migration retains Grammar 1/2 for Decimal-only output or atomically upgrades to Grammar 3 when a Fraction is required. Twenty points remain with 11p precedence, a 14p heuristic resource makespan, 3p delay, and inherited provisional forecasts `11/24d` and `7/12d`. The macro rollup is `0.583333d`, leaving `4.583333d` precedence and `6.583333d` heuristic resource makespans with 2d delay. Complete Next v3 recommends `SU_M2R_RATIONAL_DURATION_WORK_PACKAGE` in the macro and `RATIONAL_DURATION_SOURCE_MODEL` in the detail; `EXACT_DURATION_SERIALIZER` is jointly feasible and `allowed`. Active Grammar 1, accepted target Grammar 2, CLI Contract 3, public result schemas, root exports, the 27-command registry, help, Guide, and installed behavior remain unchanged. Public Contract 4 activation, unit-migration command publication, Next v4 authority, release, and external distribution remain later gates.
