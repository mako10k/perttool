...
perttool has accepted its TypeScript CLI MVP, the read-only AI Agent Guidance Registry v1 from Issue #2, and suffix-free beta releases through `v0.5.1`; the compatible `v0.5.2` JSON Schema patch is selected and in source preparation. Version `0.5.1` retains Grammar 5 and CLI Contract 6 while publishing bundled Draft 2020-12 result artifacts, additive schema discovery APIs and command, and Git 2.54 UTC commit-time compatibility from one verified tarball; a separately authorized post-acceptance operation made npm `beta=latest=0.5.1`. Version `0.5.0` remains the pre-schema Contract 6 artifact, `0.4.0` remains the Contract 5 compatibility artifact, and `0.3.0` and `0.2.0` remain the prior Contract 4 and Contract 3 artifacts, all available by explicit pin. The current `0.5.2` source implements Grammar 1/2/3/4/5 reads, registry-driven Contract 6 `help`, separate Contract 6 `guide`, temporal and governed project/task/gate/milestone/resource maintenance, exact lifecycle events, read-only project history and velocity observation, exact `project migrate-unit`, `Perttool.ProjectResult.v3`, `Perttool.MutationResult.v3` with `Perttool.GovernanceDecision.v1`, `Perttool.AnalysisResult.v4`, `Perttool.NextResult.v5`, `Perttool.UnitMigrationResult.v3`, read-only `validateOverride`, source-preserving mutation, and authorization-before-safe-write controls. The source bundles complete Draft 2020-12 artifacts for all seventeen active command-result identities plus public library-only `Perttool.OverrideDecision.v1`, exposes the additive read-only `schema` command and public catalog APIs, adds explicit full and reference-based outline/detail views, and accepts Git 2.54 strict ISO UTC `Z` commit metadata. The `0.5.1` release is durably accepted; `0.5.2` publication and Issue #5 closure remain separate boundaries. A complete and known `Perttool.NextResult.v5` with a complete temporal start-authority projection is the normal AI task-selection authority. The macro plan is complete and has no ready task. The independent English-baseline detail plan has completed and advanced all nine tasks through `ENGLISH_ACCEPTANCE`; Git commit `2001cdf` records the exact completed final-task pre-advance snapshot, `docs/process/english-baseline-acceptance.md` records the cross-surface trace, and complete NextResult v5 has no ready, recommended, or startable task.
...
Run the repository checks from the root with Node.js 22 or later. CI verifies Node.js 22 and 24. `npm run check` includes check/analyze/next validation for all twenty-three self-use plans, including the selected `0.5.2` plan, completed `0.5.1` and `0.5.0` release plans, project-actuals, owner-aware governance, scheduling-and-units macro, SU-M1/SU-M2/SU-M2R/SU-M3/SU-M4/SU-M5 detail, `0.3.0`, and `0.4.0` plans.
