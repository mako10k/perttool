...
- `src/application/`: pure services for active check/project initialization/project metadata/analyze/next, target declared-input check/project projections, capability-checked unit-migration request preparation and final source candidates, separate active/Grammar 2/Grammar 3 mutation planning, and the internal exact-Duration version-candidate boundary.
