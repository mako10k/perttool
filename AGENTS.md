...
`alpha=0.1.0-alpha.2`; and isolated public-package checks passed. Three
points remain; both makespans are 3p at `14p/1d`, and complete NextResult v5
recommends only `RELEASE_052_ACCEPTANCE`. The user's named authorization
covers the complete `0.5.2` sequence only after each predecessor gate passes;
