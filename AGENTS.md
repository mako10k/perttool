...
ADR 0004 adopts English as the repository baseline. `SURFACE_INVENTORY`, `NORMATIVE_DOCS`, `PROCESS_AND_GUIDANCE_DOCS`, `RUNTIME_MESSAGES`, and `HELP_AND_USAGE` are complete and advanced, and `PERT_PLANS` is complete at its exact pre-advance snapshot. `plans/english-baseline.pert` now has 6p remaining, matching 6p precedence and resource makespans, an observed `36p/2d` velocity, and recommends `GOLDEN_UNICODE_AUDIT`.
